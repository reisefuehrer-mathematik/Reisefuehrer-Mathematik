...
\begin{document}
\label{chap:lineare_gleichungen}
...
\begin{example}[ex:handball-quali]{}
    \parpic[r]{
        \begin{tikzpicture}[scale=0.9]
            %Ringdiagramm
            \draw[fill=green!80!black] (-3,0) -- ++ (0:2) arc [start angle=180, end angle=120, radius=1] -- ++ (120:2) arc [start angle=120, end angle=180, radius=3];
            \draw[fill=green!50!black] (1,0) -- ++ (0:2) arc [start angle=0, end angle=30, radius=3] -- ++ (-150:2) arc [start angle=30, end angle=0, radius=1];
            \draw[fill=green!65!black] (120:1) arc [start angle=120, end angle=30, radius=1] -- ++ (30:2) arc [start angle=30, end angle=120, radius=3] -- ++ (-60:2);
            \draw[fill=white] (180:1) arc [start angle=180, end angle=0, radius=1] -- cycle;
            \node at (90:0.3) {\small 48 Teams};
            %Labels
            \node[text width=1.5cm, align=center] at ($(15:2) + (0,0.3)$) {\scriptsize Süd-};
            \node[text width=1.5cm, align=center] at (15:2) {\scriptsize deutsch-};
            \node[text width=1.5cm, align=center] at ($(15:2) - (0,0.3)$) {\scriptsize land};
            \node[text width=1.5cm, align=center] at ($(75:2) + (0,0.35)$) {\scriptsize Mittel-};
            \node[text width=1.5cm, align=center] at (75:2) {\scriptsize deutsch-};
            \node[text width=1.5cm, align=center] at ($(75:2) - (0,0.35)$) {\scriptsize land};
            \node[text width=1.5cm, align=center] at ($(150:2) + (0,0.35)$) {\scriptsize Nord-};
            \node[text width=1.5cm, align=center] at (150:2) {\scriptsize deutsch-};
            \node[text width=1.5cm, align=center] at ($(150:2) - (0,0.35)$) {\scriptsize land};
            %Deutschlandkarte und Pfeile
            ...
            \draw[thick, white, fill=green!50!black] (0.6,4) arc [start angle=90, end angle=-20, radius=1.9] -- ++ (-83.43:0.447) -- ++ (43.43:0.447) arc [start angle=-20, end angle=90, radius=2.3] -- cycle;
...
Um einen Term aufzuschreiben, in dem ein Wert vorkommt, der dir unbekannt ist, wurden im Beispiel die Variablen \mtext{Plätze Süden}, \mtext{Plätze Mitte} und \mtext{Plätze Norden} verwendet. So kannst du den Term erstmal direkt aufschreiben und dir hinterher darüber Gedanken machen, was \mtext{Plätze Süden}, \mtext{Plätze Mitte} und \mtext{Plätze Norden} für Werte annehmen müssen. Wir geben unseren Unbekannten im Folgenden jedoch lieber Namen, die sich nicht nur für dieses eine Beispiel eignen, sondern die allgemein verwendet werden können, nämlich $x, y$ und $z$.
