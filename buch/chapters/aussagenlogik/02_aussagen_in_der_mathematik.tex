...
\def\drinkIcn{\statementIcon{images/trinken.png}{0.86}{}}
...
Wahrheitswert zuordnen können. Jetzt möchten wir den Aufbau von Aussagen genauer untersuchen. 
...
Im Übrigen nennen wir solch eine Aussage, die entsteht, wenn wir zwei Aussagen mit \statement{und}
verknüpfen, eine \textit{Konjunktion}.

Aussagen lassen sich aber auch durch andere Wörter, abgesehen von \statement{und} 
...
Englischen von \enquote{to connect}, was \enquote{verbinden} heißt. Das Wort \statement{und} 
...
Die Aussage \statement{Der Zaubertrank ist giftig und ich trinke ihn} besteht aus 
...
\begin{example}{Unteraussagen 2}
    \statement{Der Zaubertrank ist giftig und ich trinke ihn} ist eine Unteraussage
    von \statement{Der Zaubertrank ist giftig und ich trinke ihn}.
\end{example}
...
Wir wollen die Aussage aus letztem Beispiel \statement{Der Zaubertrank ist giftig und ich trinke ihn} kürzer notieren. Um das zu erreichen, denken wir uns Symbole für jede der zwei Unteraussagen aus. 
...
\begin{example}{Negation}
    ...
    \[\poisonIcn\]
    darstellen. Die Aussage \statement{Der Zaubertrank ist nicht giftig} drückt 
    genau das Gegenteil aus.
    In der Mathematik sagt man, dass wir die 
    Aussage \statement{Ich trinke den Zaubertrank} durch das Einbauen des 
    Konnektors \statement{nicht} \textit{negiert} haben. Die Aussage \statement{Ich trinke den Zaubertrank nicht} nennen wir dann die \textbf{Negation} zu der Aussage \statement{Ich trinke den Zaubertrank}.
    \bigskip
    
    Um eine Negation symbolisch zu notieren, streichen wir das Bild der Aussage, die 
    wir negieren wollen, einfach durch. Die Aussage  \statement{Der Zaubertrank ist nicht giftig} können wir also wie folgt schreiben:
...
\begin{example}{Genau dann, wenn}
    ...
    \begin{enumerate}
        ...
        \item Der Trank ist giftig (verleiht keine Superkräfte) und wir trinken den Trank nicht
             ...
             \[\statement{ \statementIcon{images/super_power.png}{0.86}{n} und \statementIcon{images/trinken.png}{0.86}{n}}\]
    \end{enumerate}
    ...
    dem Konnektor
    \statement{genau dann, wenn} (wie in diesem Beispiel), dann drücken wir damit aus, 
...
\begin{example}{Formalisierung einer Aussage}
    ...
    Unteraussage zu tun hat. Es bietet sich daher an, die Unteraussage \statement{Das Menü enthält einen Burger} mit einem Burgersymbol abzukürzen. Wir verwenden folgende Abkürzungen:
   ...
   \[\burgerIcn \textbf{ und } \Bigl(\saladIcn \textbf{ oder } \friesIcn\Bigr)\]
    ...
    Das Zeichen für \statement{und} ist $\land$ und das Zeichen 
    für \statement{oder} ist $\lor$. Ersetzen wir \statement{und}, \statement{oder} in der Aussage durch die entsprechenden logischen Symbole, dann erhalten wir:
...
\begin{enumerate}
    ...
    Zum Beispiel ist das Symbol zu \statement{und} ein $\land$ und zu \statement{oder} ein $\lor$. 
\end{enumerate}
...
\begin{example}{Formalisierungen}
    ...
    \begin{tabularx}{.8\linewidth}{@{}>{\collectcell\statement}X<{\endcollectcell}@{\hspace{1cm}}r@{}c@{}l@{}}
        ...
        \textbf{Wenn} der Trank \textbf{nicht} giftig ist, \textbf{dann} trinke ich ihn. & ($\lnot$\poisonIcn)&$\limplies$& $\drinkIcn$\\
        Ich trinke den Trank \textbf{genau dann, wenn} er Superkräfte verleiht. & \drinkIcn&$\liff$& \superPwrIcn\\
        \bottomrule
    \end{tabularx}
    \end{center}
\end{example}
...
\begin{nutshell}{Aussagen in der Mathematik}
   ...
   \begin{center}
       \begin{tabular}{cc}\toprule
            ...
            nicht & $\lnot$\\
            wenn, dann& $\limplies$\\
            genau dann, wenn&  $\liff$\\\bottomrule
        \end{tabular}
    \end{center}
\end{nutshell}
