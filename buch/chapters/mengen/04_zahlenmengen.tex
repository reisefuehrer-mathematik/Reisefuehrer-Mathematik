...
\begin{document}
	...
	\todo{Beispiel ist hier noch unbeholfen, evtl auch ein Bildchen dazu}
	...
	\todo{Bild vielleicht in bunt?}
	\begin{figure}[h]
		\centering
		\begin{tikzpicture}[scale=5]
			\draw (-1.1,0)--(1.1,0);
			\foreach \x in {-1,0,1} \draw (\x,8pt)--(\x,-8pt) node[below] {$\x$};		
			...
			%		\node at (1.1,0) [right] {\ldots};
			%		\node at (-1.1,0) [left] {\ldots};
		\end{tikzpicture}
	...
	In manchen Fällen ist die Sache klar. Zum Beispiel ist $\sqrt{9}=3$ eine natürliche Zahl und liegt somit in \Rational. Andere Wurzeln sind leider nicht so eindeutig, wie etwa $\sqrt{2}$. Der Wert von $\sqrt{2}$ liegt irgendwo zwischen $1$ und $2$ und ist damit nicht mehr natürlich. 
	...
	\todo{Korollar? Aussage direkt vorziehen und dann beweisen?}
	...
	\begin{nutshell}{Zahlenmengen}
		...
		\begin{center}
			\begin{tikzpicture}[scale=1.3]
				\draw (0.7,0) ellipse (2 and 0.8);
				\node at (2.5,0) {$\mathbb{R}$};
				\draw (0.5,0) ellipse (1.6 and 0.7);
				\node at (1.9,0) {$\mathbb{Q}$};
				\draw (0.4,0) ellipse (1.2 and 0.6);
				\node at (1.3,0) {$\mathbb{Z}$};
				\draw (0.2,0) ellipse (0.8 and 0.5);
				\node at (0.7,0) {$\mathbb{N}_0$};
				\draw (0,0) circle[radius=0.4];
				\node at (0,0) {$\mathbb{N}$};
				%			\draw (1,0) ellipse (2.4 and 0.9);
				%			\node at (3.1,0) {$\mathbb{C}$};
			\end{tikzpicture}
		\end{center}
	\end{nutshell}
\end{document}
