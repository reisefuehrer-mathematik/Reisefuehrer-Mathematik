...
\begin{document}
	...
	\newcommand{\inclusionDiagram}[1]{
		\begin{tikzpicture}[draw = black!50, font = \small, scale = 0.8]
			\ifnum#1=4
				\draw[fill = black!10] (0, 2.75) ellipse (1.8 and 3.75);
				\node[left, maincolor] at (0, 5.8) {\Real};
				\foreach \i/\j in {(-0.9, 5.2)/\sqrt{2}, (0.7, 5.5)/\pi, (1.1, 4.75)/\sqrt{3}} {
					\node[font = \tiny] at \i {$\j$};
				}
			\fi	
			\ifnum#1>2		
				\draw[fill = black!13] (0,2) ellipse (1.6 and 3);
				\node[left, maincolor] at (0,4) {\Rational};
				\foreach \i/\j in {(-0.9, 3.5)/\frac{2}{3}, (0.25, 4.25)/\frac{1}{2}, (1.1, 3.2)/\frac{7}{8}} {
					\node[font = \tiny] at \i {$\j$};
				}
			\fi
			\ifnum#1>1			
				\draw[fill = black!16] (0,1.2) ellipse (1.4 and 2.2);
				\node[left, maincolor] at (0, 2.6) {\Integer};
				\foreach \i/\j in {(0,3)/-1, (-1, 2)/-2, (1, 1.75)/-3, (0.5,2.3)/-4} {
					\node[font = \tiny] at \i {$\j$};
				}
			\fi
			\ifnum#1>0			
				\draw[fill = black!19] (0,0.5) ellipse (1.2 and 1.5);
				\node[left, maincolor] at (0,1.4) {$\Natural_0$};
				\node[font = \tiny] at (0.5,1.25) {$0$};			
				\draw[fill = black!22] (0,0) circle[radius = 1];
				\node[left, maincolor] at (0,0) {\Natural};			
	...
	\picchangemode
	Wir nutzen Mengen nicht nur, um einfache Alltagsobjekte in Kategorien zu ordnen, wir können auch Mengen verschiedener Zahlen betrachten. Eine besondere Rolle nehmen dabei die Mengen der verschiedenen Zahlentypen ein. Diese hast du nach und nach kennengelernt, als du erst in der Grundschule das Zählen und später das Rechnen mit negativen Zahlen und Bruchzahlen gelernt hast. Wir wollen die verschiedenen Zahlenbereiche nun beschreiben, ihnen einen Namen geben und auch ein Symbol zuordnen, durch das wir immer wissen, welche Zahlen gemeint sind.
	
	\parpic{
		\inclusionDiagram{1}
	}\picskip{7}
	Als erstes hast du die Zahlen kennengelernt, die wir im Alltag zum Zählen verwenden. Das sind die Zahlen $1$, $2$, $3$ und so weiter. Zusammengenommen ergeben sie die Menge der \textbf{natürlichen Zahlen}, für die wir auch einfach \Natural schreiben. Wenn man die Eier in seinem Kühlschrank zählen möchte, verwendet man also genau die natürlichen Zahlen (ein Ei, zwei Eier, drei Eier,$\ldots$). Damit schreiben wir für die natürlichen Zahlen 	
	\[\Natural = \{1,2,3,\ldots\}.\]
	Eine gesonderte Rolle kommt der Zahl $0$ zu. Leider sind sich Mathematiker nicht einig darüber, ob die 0 als natürliche Zahl zählt oder nicht. Wir zählen die $0$ hier nicht als natürliche Zahl (es gibt kein nulltes Ei). Möchte man dennoch zusätzlich die $0$ beim Zählen betrachten (ich habe sechs Eier in meinem Kühlschrank und lege kein weiteres dazu), so schreibt man stattdessen $\Natural_0$. Diese Menge beinhaltet also die Zahlen $0$, $1$, $2$, $3$, $\ldots$ und wir schreiben
	\[\Natural_0 = \{0,1,2,\ldots\}.\]
	
%	\begin{example}{}
%		Die Zahlen 1, 5, 17 und 14159 sind natürliche Zahlen und liegen somit in \Natural.		
%	\end{example}

	Wir wollen aber nicht immer nur aufzählen, also addieren, sondern auch Zahlen voneinander abziehen. Innerhalb der natürlichen Zahlen geht die Subtraktion so lange gut, bis wir eine größere von einer kleineren Zahl abziehen wollen. Zum Beispiel ist $3-7=-4$, was negativ und somit keine natürliche Zahl mehr ist. Das Rechnen mit negativen Zahlen hast du im Kapitel über \mayberef{ganze Zahlen} in diesem Buch kennengelernt. Die Zahlen, die so entstehen können, also etwa 3, 7 und $-4$, nennen wir die \textbf{ganzen Zahlen}.
	...
	Insgesamt sind diese Zahlenmengen also jeweils echt ineinander enthalten (man sagt, sie bilden eine Kette von echten Teilmengen). Im rechts abgebildeten Mengendiagramm siehst du zum Beispiel, dass die Menge der ganzen Zahlen wie im Abschnitt über \mayberef{Teilmengen} in der Menge der rationalen Zahlen enthalten ist. In der Menge der ganzen Zahlen ist wiederum die Menge der natürlichen Zahlen enthalten. 
	...
	In diesem Abschnitt haben wir vier Zahlenmengen definiert, mit denen wir am häufigsten zu tun haben. Ihre Notation und Definition fassen wir wie folgt zusammen.
	...
	Wir haben jetzt diejenigen Zahlen untersucht, mit denen wir im Alltag am meisten zu tun haben. Diese Zahlen können wir nun ihrem Wert entsprechend auf der \mayberef{Zahlengerade} markieren. Jeder noch so kleine Punkt auf dieser Geraden (markiert oder nicht) entspricht einer Zahl.
	...
	Hier siehst du einen Ausschnitt einer beispielhaften Zahlengeraden. Eingezeichnet sind die ganzen Zahlen $-1,\ 0$ und $1$ sowie ein paar ausgewählte (aber nicht alle) Bruchzahlen, die dazwischen liegen. Die Gerade kann auf beiden Seiten weiter fortgeführt werden.
	...
	\end{proof}
	\todo{Wir nutzen hier, dass $x^2$ gerade gdw. $x$ gerade. Auf Satz verweisen}

	\parpic{
		\inclusionDiagram{4}
	}
	Tatsächlich sind Wurzeln von Primzahlen niemals rational. Auch andere Zahlen, wie die Kreiszahl $\pi$, die du später im Kapitel über \mayberef{Kreise} kennenlernst, sind nicht in \Rational enthalten. Auf unserer Zahlengeraden liegen diese Zahlen in den Lücken zwischen rationalen Zahlen. Die Menge, die alle Zahlen der Zahlengeraden enthält, nennen wir die Menge der \textbf{reellen Zahlen} \Real, die wir an dieser Stelle nicht formal definieren wollen. Jeder beliebige Punkt auf der Zahlengeraden ist somit eine reelle Zahl.
	...
	\[\Natural \subset \Natural_0 \subset \Integer \subset \Rational \subset \Real\]
	...
	\begin{nutshell}{Zahlenmengen}
		...
		\begin{itemize}
			...
			\item $\Integer = \left\{\ldots, \ -3, \ -2, \ -1, \ 0, \ 1, \ 2, \ 3, \ldots \right\}$ die Menge der \textbf{ganzen Zahlen}
			\item $\Rational = \left\{ \frac{a}{b} \mid a, \ b \in \Integer \text{ und } b \neq 0\right\}$ die Menge der \textbf{rationalen Zahlen}
			\item \Real die Menge der \textbf{reellen Zahlen}, die aus allen Zahlen auf der Zahlengeraden besteht.
		\end{itemize}
		...
		\begin{center}
			\begin{tikzpicture}[scale=1.3, draw = black!50]
				\draw[fill = black!10]  (1.2,0) ellipse (1.6 and 0.8);
				\node at (2.5,0) {$\mathbb{R}$};
				\draw[fill = black!13]  (0.9,0) ellipse (1.3 and 0.7);
				\node at (1.9,0) {$\mathbb{Q}$};
				\draw[fill = black!16]  (0.6,0) ellipse (1 and 0.6);
				\node at (1.3,0) {$\mathbb{Z}$};
				\draw[fill = black!19]  (0.3,0) ellipse (0.7 and 0.5);
				\node at (0.7,0) {$\mathbb{N}_0$};
				\draw[fill = black!22] (0,0) circle[radius=0.4];
				\node at (0,0) {$\mathbb{N}$};
			\end{tikzpicture}
		\end{center}
	\end{nutshell}
